    ...
    jQuery.fn.IMxSmartReload = function (options) {
        ...
        jQuery('.block-imx-ads').find(".imx_ads div[id^='sas_'], .imx_ads div[id^='eplAdDivbox_'], .imx_ads div[id^='imx_ads_'], .imx_ads iframe").each(function () {
            if (jQuery(this).height() > 100) {
                ...
                jQuery(this).parent().css("min-with", "" + jQuery(this).width() + "px");
                    ...
                    if (!fullscreenEnabled) {
                        ...
                        jQuery('.block-imx-ads').each(function () {
                            ...
                            jQuery(this).find(".imx_ads").each(function () {
                                if (jQuery(this).height() > 100) {
                                    ad_format = jQuery(this).attr("id");
                                    ad_format = jQuery(this).attr("id").replace('imx_ads_', '');
                                    console.log(ad_format);
                                    ad_clone = jQuery(this).clone();
                                    jQuery(this).children().remove();
                                    //parent_block.append(ad_clone);
                                    createBannerAsync("http://cf.imx.static.files.invent.mx/imx_ads_async.html",site_id,sas_pageid, ad_format, sas_target, jQuery(this).attr("id"));
